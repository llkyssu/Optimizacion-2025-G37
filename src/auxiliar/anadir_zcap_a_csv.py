...
default_zcap = int(round(sum(ZCAPS.values()) / len(ZCAPS)))

input_folder = 'datos_por_comuna_csv'
output_folder = 'datos_por_comuna_csv_zcap'
...
for path in glob.glob(f"{input_folder}/*.csv"):
    ...
    df['Zcap'] = df['tipo_osm'].map(ZCAPS)
    df['Zcap'] = df['Zcap'].fillna(default_zcap).astype(int)
    out_path = os.path.join(output_folder, normalizar_nombre(os.path.basename(path)))
    df.to_csv(out_path, index=False)
    print(f"Archivo exportado con Zcap: {out_path}")
